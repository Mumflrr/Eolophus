...
def plan_node(state: PipelineState) -> dict:
    ...
    for attempt in range(max_attempts):
        try:
            ...
            log.warning("Planner JSON validation failed (attempt %d/%d): %s", attempt + 1, max_attempts, str(e))
            if attempt == max_attempts - 1:
                raise RuntimeError(f"Planner failed to produce valid PlanSpec after {max_attempts} attempts.") from e
            
            # Feed the exact schema error back to the model
            messages.append({"role": "assistant", "content": "I provided malformed JSON."})
            messages.append({
                "role": "user", 
                "content": f"Your previous output failed Pydantic validation with this error:\n{str(e)}\n\nPlease ensure your PlanSpec is strictly valid JSON."
            })
